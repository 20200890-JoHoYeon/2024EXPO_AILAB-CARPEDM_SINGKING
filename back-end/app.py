...
def index():
    ...
    if session['user_id']:
        ...
        if user_vocal_data:
            ...
            # user_vocal_data가 비어 있을 경우 기본값 처리
            pitch_score = beat_score = pronunciation_score = None
            last_week_pitch = last_week_beat = last_week_pronunciation = None
...
def vocal_analysis():
    ...
    beat_score = round(va.score_cover()['accuracy'], 2) +40
    pronunciation_score = (va.pronunciation_score() or 0.00) + 80
   
    
    total_score = (pitch_score+beat_score+pronunciation_score)/3
    # total_score = 14.2
    print(type(pronunciation_score), '타입임')
...
def ai_tone():
    ...
    # user_id가 세션에 저장되어 있다고 가정
    user_id = session.get('user_id')  # 세션에서 user_id 가져오기

    if user_id:
        # DB 업데이트 함수 호출
        db.update_user_tone(user_id, tone)
...
#     # 임시로 정보 보냅니다
#     return {'level':0, 'pitch':0, 'beat':0, 'tone':'진단필요'}
